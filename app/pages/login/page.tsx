...
function LoginContent() {
  ...
  const updateUserContext = (authUser: { id: string; email?: string | null; user_metadata?: Record<string, any> }) => {
    setUser({
      ...user,
      name: authUser.user_metadata?.first_name || authUser.email?.split("@")[0] || user.name,
      id: authUser.id,
    });
  };
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    if (!isSupabaseConfigured || !supabase) {
      setLoginMsg({ text: "Login isn't available right now. Please try again later.", type: "error" });
      return;
    }
    setLoggingIn(true);
    setLoginMsg(null);
    try {
      ...
      if (error) {
        ...
        setLoginMsg({ text: "Logged in successfully", type: "success" });
        localStorage.setItem("userEmail", data.user.email ?? loginEmail);
        updateUserContext(data.user);
        setTimeout(() => router.push("/pages/job-requisitions"), 600);
      }
    } catch (err) {
      console.error("Login failed:", err);
      setLoginMsg({ text: "Something went wrong. Please try again.", type: "error" });
    }
    setLoggingIn(false);
  };
  ...
  const handleRegister = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!isSupabaseConfigured || !supabase) {
      setRegMsg({ text: "Registration isn't available right now. Please try again later.", type: "error" });
      return;
    }
    setRegistering(true);
    setRegMsg(null);
    try {
      ...
      if (error) {
        ...
        // Email confirmation disabled: Supabase already signed the user in.
        setRegistered(true);
        localStorage.setItem("userEmail", data.user.email ?? regEmail);
        updateUserContext(data.user);
        setRegMsg({ text: "Successfully registered", type: "success" });
        setTimeout(() => router.push("/pages/job-requisitions"), 600);
      } else {
        // Email confirmation required before the user can sign in.
        setRegistered(true);
        setRegMsg({ text: "Successfully registered. Check your email to confirm your account before signing in.", type: "success" });
      }
    } catch (err) {
      console.error("Register failed:", err);
      setRegMsg({ text: "Something went wrong. Please try again.", type: "error" });
    }
